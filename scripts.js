        ...
        reader.onload = function(e) {
            ...
            img.onload = function() {
                if (img.width === 32 || img.width === 64 || img.width === 128) {
                    ...
                    uploadButton.remove(); // Remove uploadButton
                    document.querySelector('.upload-container').removeChild(uploadButton); // Remove plus-icon
                } else {
...
function addToSkinPack() {
    ...
    // Show text below preview skin (jika belum ditambahkan sebelumnya)
    const uploadContainer = document.querySelector('.upload-container');
    if (!uploadContainer.querySelector('p')) {
        const uploadText = document.createElement('p');
        uploadText.textContent = 'Click your skin to upload another';
        uploadText.style.fontSize = '12px';  // Mengatur ukuran font menjadi lebih kecil
        uploadText.style.fontStyle = 'italic';  // Mengatur font menjadi italic/miring
    ...
    function generateSkinPack(skinPackName) {
        ...
        skinData.forEach((skin) => {
            const skinFileName = `texture/${skin.localization_name}.png`;
